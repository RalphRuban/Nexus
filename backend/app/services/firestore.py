...
# When a Firestore request fails at runtime -- e.g. the Spark free tier
# exhausts its daily read quota and returns 429 -- we degrade to the
# in-memory mock store so the demo keeps serving. On memory-limited hosts
# (Render's ~512 MB free plan) loading the full seed can OOM, so we switch
# to a compact dataset (app.data.lite_data) kept small on purpose.
_USE_LITE = False
_FIRESTORE_FAILED = False


def _mark_firestore_failed():
    global _USE_LITE, _FIRESTORE_FAILED
    _FIRESTORE_FAILED = True
    _USE_LITE = True
    logger.warning(
        "Firestore unavailable (likely quota exceeded); "
        "falling back to lightweight in-memory store for this process."
    )
...
# Lazy mock data
#
# The full seed data (``app.data.mock_data -> app.data.real_data``) is a
# large module whose import materializes ~3,660 weather records and 198
# wards as live Python objects (~200-400 MB RSS). In normal mock mode we
# only ever need it if USE_FIRESTORE is off. When Firestore fails at
# runtime we instead use the compact ``app.data.lite_data`` so we never
# blow the memory budget on a small host. Both are deferred until first
# access to keep the process footprint small at startup.
# ---------------------------------------------------------------------------
...
def _load_mock_data() -> dict:
    """Import the seed data module once and cache the raw collections.

    Returns the lightweight collections after a Firestore failure; the
    full collections otherwise.
    """
    global _mock_data_cache

    if _mock_data_cache is None:
        if _USE_LITE:
            from app.data.lite_data import (  # compact import - deferred
                ACTIVITY,
                HOSPITALS,
                INCIDENTS,
                INCIDENT_REPORTS,
...
def reset_store() -> int:
    ...
    number of documents loaded. Does nothing in active Firestore mode."""
    if USE_FIRESTORE and not _FIRESTORE_FAILED:
        return 0
